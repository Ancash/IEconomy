...
public class IEconomyPacket implements Serializable{
	...
	public static Packet asPacket(short header, UUID uuid, double value, long timestamp) {
		return new Packet(header).setSerializable(new IEconomyPacket().setUUID(uuid).setValue(value)).setLong(timestamp);
	}
}